...
export default function TelaDeAtualizarCliente() {
  ...
  function notificacaoUpdateCliente(mensagem) {
    setMsg(mensagem);
    setAbrir(true);
  }

  async function atualizarCliente() {
    if (validaCampoText([name, username, password, response, question])) {
      user.phone[0] = phone;
      if (userDistrict) {
        user.address[0] = {
      ...
      await Api.put(`users/${user._id}`, userUpdate).then(response => {
        notificacaoUpdateCliente('Atualizado com Sucesso!');
      });
      await setProgresso(false);
      await signOut();
    } else {
      notificacaoUpdateCliente('Preencha todos os dados do cliente! O endereço é opcional.');
    }
  }
